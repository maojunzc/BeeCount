...
class BillCreationService {
  ...
  Future<int?> createBillTransaction({
    ...
    if (result.aiType != null &&
        result.aiType!.isNotEmpty &&
        (result.aiType == 'income' ||
            result.aiType == 'expense' ||
            result.aiType == 'transfer')) {
      ...
      if (hasPlusSign) {
        ...
        // 同时出现收入和支出关键字，优先判断支出（记账场景支出更常见，避免误将支出记为收入）
        transactionType = 'expense';
        typeSource = '关键字冲突(支出优先)';
