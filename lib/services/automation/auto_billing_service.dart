      ...
      if (result.amount != null && result.amount!.abs() > 0) {
        ...
        try {
          ...
          if (transactionId != null) {
            ...
            if (showNotification) {
              final amountStr = result.amount!.abs().toStringAsFixed(2);
              final typeLabel = result.aiType == 'income' ? '收入' : '支出';
              await _showNotification(
                id: notificationId,
                title: '✅ 自动记账成功 ¥$amountStr',
                body: result.note != null
                    ? '$typeLabel: ${result.note}'
                    : '已自动创建${typeLabel}记录',
              );
            }
      ...
      if (txId != null) {
        ...
        if (showNotification) {
          await _showNotification(
            id: notificationId,
            title: '✅ 记账成功',
            body: '已自动创建记账记录: ¥${ocrResult.amount}',
          );
        }
        return txId;
      } else {
